    ...
    */
    ...
    if (inlierEdgeRatio >= 0.5)// take up 0.8 of edge points
    {
        ...
        for (int k = 0; k < num; k++)
        {
            ...
            if (signMid * signCenter > 0)
            {
                spanAngle += (2 * CV_PI - angle);    // superior arc
            }
            else
            {
                spanAngle += angle;    // inferior arc
            }
        }//endfor
    }//endif
    *pr = R;
...
vector<Circles> circleEstimateGroupedArcs(vector<vector<Point>> groupedArcs, vector<Vec3f>recordOR, vector<vector<Point>> groupedArcsThreePt, float T_inlier)
{
    vector<Circles> addCircles;
    for (int i = 0; i < groupedArcs.size(); i++)
    {
...
vector<Circles> circleEstimateClosedArcs(vector<vector<Point>> closedArcs)
{
...
// draw fitted circles
vector<Circles> drawResult(bool onImage, Mat srcImg, vector<Circles> circles)
{
    Mat colorImage;
    ...
    for (int i = 0; i < circles.size(); i++)
    {
        ...
        else
        {
            center = Point2f(circles[i].xc, circles[i].yc);
            r = circles[i].r;
            circle(colorImage, center, r, Scalar(0, 0, 255), 1, LINE_AA);
        }
    }
    return circles;
}
